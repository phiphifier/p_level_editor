...
public class MenuAddEntityDefinition extends Menu implements StringInputFieldListener {
	...
	private SplashString currentSplashString;
		// MENU PROPERTIES
	private static final String MENU_TITLE = "Enter entity details:";
	...
	// CONSTRUCTOR
	public MenuAddEntityDefinition(Menu owner) {
		super(owner);
		...
		createButton = addButtonWithMouseHoverTip(BUTTON_ID_CREATE, BUTTON_CREATE_X, BUTTONS_Y, BUTTONS_WIDTH, BUTTONS_HEIGHT, "Create", false, "Add this entity definition", HorizontalAlignment.RIGHT);
		createInputFields();
	}
	...
	public void buttonLeftReleased(int buttonID) {
		switch(buttonID) {
			case BUTTON_ID_CREATE: {
				...
				if (EntityDefinitionManager.addNewEntityDefinition(currentName, currentID)) {
					...
					MenuEntitySelect myOwner = (MenuEntitySelect)owner;
					myOwner.updateEntityDefinitionButtons();
					createInputFields();
				}
				else {
